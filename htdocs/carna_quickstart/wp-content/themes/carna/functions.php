 ...
 */
/**
 * Carna only works in WordPress 4.7 or later.
 */
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'carna') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'carna') ?></button>
    </form>
    <?php
}
...
function ftc_setup() {
     ...
     * to change 'carna' to the name of your theme in all the template files.
     */
    load_theme_textdomain('carna');
    ...
    /* Translation */
    load_theme_textdomain('carna', get_template_directory() . '/languages');
    ...
    register_nav_menus(array(
        'primary' => __('Primary Navigation', 'carna'),
        'vertical' => __('Vertical Navigation', 'carna'),
    ));
    ...
    $starter_content = array(
        ...
        'attachments' => array(
            'image-espresso' => array(
                'post_title' => _x('Espresso', 'Theme starter content', 'carna'),
                'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
            ),
            'image-sandwich' => array(
                'post_title' => _x('Sandwich', 'Theme starter content', 'carna'),
                'file' => 'assets/images/sandwich.jpg',
            ),
            'image-coffee' => array(
                'post_title' => _x('Coffee', 'Theme starter content', 'carna'),
        ...
        'nav_menus' => array(
            ...
            'social' => array(
                'name' => __('Social Links Menu', 'carna'),
                'items' => array(
                    'link_yelp',
                    'link_facebook',
                    'link_twitter',
...
function ftc_fonts_url() {
     ...
     */
    $dosis = _x('on', 'Dosis font: on or off', 'carna');
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
            /* translators: %s: Name of current post */ sprintf(__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'carna'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = __('Expand child menu', 'carna');
        $ftc_l10n['collapse'] = __('Collapse child menu', 'carna');
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        if ($search_for_product) {
            ...
            $post_type = 'product';
            $placeholder_text = esc_html__('Search...', 'carna');
        } else {
            $taxonomy = 'category';
            $post_type = 'post';
            $placeholder_text = esc_html__('Search', 'carna');
        ...
        $options = '<option value="">' . esc_html__('All categories', 'carna') . '</option>';
		...
		<form method="get" id="searchform' . $rand . '" action="' . esc_url(home_url('/')) . '">
		 ...
		 <div class="ftc_search_ajax">
			 ...
			 <input type="submit" title="' . esc_attr__('Search', 'carna') . '" id="searchsubmit' . $rand . '" value="' . esc_attr__('Search', 'carna') . '" />
...
if (!function_exists('ftc_ajax_search')) {
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'carna'), $results->found_posts);
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'carna');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'carna')
            , '404' => esc_html__('Error 404', 'carna')
            , 'tagged' => esc_html__('Tagged ', 'carna')
            , 'author' => esc_html__('Articles posted by ', 'carna')
            , 'page' => esc_html__('Page', 'carna')
            , 'portfolio' => esc_html__('Portfolio', 'carna')
        );
