    ...
    if ($product->managing_stock()) {
        if ($product->is_in_stock() && $product->get_stock_quantity() > get_option('woocommerce_notify_no_stock_amount')) {
            switch (get_option('woocommerce_stock_format')) {
                case 'no_amount' :
                    $availability = esc_html__('In stock', 'carna');
                    break;
                case 'low_amount' :
                    if ($product->get_stock_quantity() <= get_option('woocommerce_notify_low_stock_amount')) {
                        $availability = sprintf(esc_html__('Only %s left in stock', 'carna'), $product->get_stock_quantity());
                        if ($product->backorders_allowed() && $product->backorders_require_notification()) {
                            $availability .= ' ' . esc_html__('(can be backordered)', 'carna');
                        }
                    } else {
                        $availability = esc_html__('In stock', 'carna');
                    }
                    break;
                default :
                    $availability = sprintf(esc_html__('%s in stock', 'carna'), $product->get_stock_quantity());
                    if ($product->backorders_allowed() && $product->backorders_require_notification()) {
                        $availability .= ' ' . esc_html__('(can be backordered)', 'carna');
                    }
                    break;
            ...
            $availability = esc_html__('Available on backorder', 'carna');
            $class = 'available-on-backorder';
        } elseif ($product->backorders_allowed()) {
            $availability = esc_html__('In stock', 'carna');
            $class = 'in-stock';
        } else {
            $availability = esc_html__('Out of stock', 'carna');
            $class = 'out-of-stock';
        }
...
function ftc_template_loop_categories(){
 global $product;        
    $categories_label = esc_html__('Categories: ', 'carna');
...
function ftc_shop_top_content_widget_area_button() {
    ...
    if (is_active_sidebar('product-category-top-content') && $smof_data['ftc_prod_cat_top_content']) {
        ?>
        <div class="prod-cat-show-top-content-button"><a href="#"><?php esc_html_e('Filter', 'carna') ?></a></div>
        <?php
    }
}
...
function ftc_template_single_navigation() {
    ...
    <div class="social-sharing">
        <div class="print">
            <a href="javascript:window.print()" rel="nofollow"><i class="fa fa-print"></i><?php esc_html_e('Print', 'carna') ?></a>
        </div>
        <div class="email">
            <a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>">
                <i class="fa fa-envelope"></i>
    <?php esc_html_e('Email to a Friend', 'carna') ?>
            </a>
        </div>
...
function ftc_template_after_single_social_sharing() {
    ...
    function ftc_template_single_meta() {
       ...
       if ($smof_data['ftc_prod_cat']) {
            echo wc_get_product_category_list($product->get_id(), ', ', '<div class="caftc-link"><span>' . _n('Category:', 'carna', count($product->get_category_ids()), 'carna') . ' ', '</span><span class="cat-links"></span></div>');
        }
        if ($smof_data['ftc_prod_tag']) {
            echo wc_get_product_tag_list($product->get_id(), ', ', '<div class="tags-link"><span>' . _n('Tag:', 'Tags:', count($product->get_tag_ids()), 'carna') . ' ', '</span><span class="tag-links"></span></div>');
        }
    ...
    function ftc_template_single_sku() {
        ...
        if (wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type('variable') )) {
            echo '<div class="sku-wrapper product_meta">' . esc_html__('Sku: ', 'carna') . '<span class="sku" itemprop="sku">' . (( $sku = $product->get_sku() ) ? $sku : esc_html__('N/A', 'carna')) . '</span></div>';
        }
    }
    ...
    function ftc_template_single_availability() {
        ...
        $product_stock = ftc_product_get_availability();
        $availability_text = empty($product_stock['availability']) ? esc_html__('In Stock', 'carna') : esc_attr($product_stock['availability']);
        ?>	
...
if (class_exists('YITH_WCWL')) {
    ...
    function ftc_add_wishlist_button_to_product_list() {
        ...
        . '" class="add_to_wishlist wishlist" ><i class="fa fa-heart"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'carna') . '</span></a>';
        ...
        echo '<a href="' . esc_url($wishlist_url) . '"><i class="fa fa-heart-o"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'carna') . '</span></a>';
        echo '</span>';
...
function ftc_cart_continue_shopping_button() {
    echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="button button-secondary">' . esc_html__('Continue Shopping', 'carna') . '</a>';
}
?>