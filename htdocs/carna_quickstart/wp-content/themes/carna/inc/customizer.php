...
function ftc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'carna' ),
		'choices'  => array(
			'light'  => __( 'Light', 'carna' ),
			'dark'   => __( 'Dark', 'carna' ),
			'custom' => __( 'Custom', 'carna' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_control( 'page_layout', array(
		'label'       => __( 'Page Layout', 'carna' ),
		'section'     => 'theme_options',
		'type'        => 'radio',
		'description' => __( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'carna' ),
		'choices'     => array(
			'one-column' => __( 'One Column', 'carna' ),
			'two-column' => __( 'Two Column', 'carna' ),
		),
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( __( 'Front Page Section %d Content', 'carna' ), $i ),
			'description'    => ( 1 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'carna' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
