...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'backup':
					...
					if(!isset($backup['backup_log'])) {
						$log = esc_html__('No backups yet', 'carna');
					} else {
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ', 'carna').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="'.esc_html__('Backup Options', 'carna').'">'.esc_html__('Backup Options', 'carna').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="'.esc_html__('Restore Options', 'carna').'">'.esc_html__('Restore Options', 'carna').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="'.esc_html__('Restore Options', 'carna').'">'.esc_html__('Import Options', 'carna').'</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.esc_html__('Upload', 'carna').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.esc_html__('Remove', 'carna').'</span>';
		}
		else 
		{
