...
function ftc_of_option_setup()	
{
	global $of_options, $options_machine;
	$ftc_data = of_get_options();
	$options_machine = new Options_Machine($of_options);
	$ftc_data = ftc_array_atts($options_machine->Defaults, $ftc_data);
	ftc_of_save_options($ftc_data);	
}

function ftc_of_save_options($data){
	foreach ( $data as $k=>$v ) {
		if (is_array($v)) {
			foreach ($v as $key=>$val) {
				if ($key != $k && $v[$key] == $val) {
