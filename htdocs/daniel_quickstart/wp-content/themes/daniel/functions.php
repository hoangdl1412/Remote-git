<?php
/**
 * Daniel functions and definitions
 *
...
/**
 * Daniel only works in WordPress 4.7 or later.
 */
...
?>
	<form action="<?php echo admin_url( 'widgets.php' ); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'daniel') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'daniel') ?></button>
    </form>
<?php
}
...
function ftc_setup() {
	 ...
	 * If you're building a theme based on Daniel, use a find and replace
	 * to change 'daniel' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'daniel' );
	...
	$GLOBALS['content_width'] = 1200;
        ...
        /* Translation */
        load_theme_textdomain( 'daniel', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary'    => __( 'Primary Navigation', 'daniel' ),
		'vertical' => __( 'Vertical Navigation', 'daniel' ),
	) );
	...
	$starter_content = array(
		...
		'attachments' => array(
			'image-espresso' => array(
				'post_title' => _x( 'Espresso', 'Theme starter content', 'daniel' ),
				'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
			),
			'image-sandwich' => array(
				'post_title' => _x( 'Sandwich', 'Theme starter content', 'daniel' ),
				'file' => 'assets/images/sandwich.jpg',
			),
			'image-coffee' => array(
				'post_title' => _x( 'Coffee', 'Theme starter content', 'daniel' ),
		...
		'nav_menus' => array(
			...
			'social' => array(
				'name' => __( 'Social Links Menu', 'daniel' ),
				'items' => array(
					'link_yelp',
					'link_facebook',
					'link_twitter',
	...
	/**
	 * Filters Daniel array of starter content.
	 *
	 * @since Daniel 1.1
	 *
...
function ftc_content_width() {
	...
	/**
	 * Filter Daniel content width of the theme.
	 *
	 * @since Daniel 1.0
	 *
...
function ftc_excerpt_more( $link ) {
	...
	$link = sprintf( '<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>',
		...
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'daniel' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
    global $smof_data, $ftc_page_datas;
        ...
        wp_register_style( 'font-swiss', get_template_directory_uri() . '/assets/css/font-swiss721.css' );
        wp_enqueue_style( 'font-swiss' );
	...
	if ( has_nav_menu( 'top' ) ) {
		...
		$ftc_l10n['expand']         = __( 'Expand child menu', 'daniel' );
		$ftc_l10n['collapse']       = __( 'Collapse child menu', 'daniel' );
...
if( !function_exists('ftc_get_search_form_by_category') ){
	function ftc_get_search_form_by_category(){
		...
		if( $search_for_product ){
			...
			$post_type = 'product';
			$placeholder_text = esc_html__('Search ...', 'daniel');
		}
		else{
			$taxonomy = 'category';
			$post_type = 'post';
		...
		$options = '<option value="">'.esc_html__('All categories', 'daniel').'</option>';
		...
		<div class="ftc_search_ajax">
			 ...
			 <input type="submit" title="' . esc_attr__( 'Search', 'daniel' ) . '" id="searchsubmit' . $rand . '" value="' . esc_attr__( 'Search', 'daniel' ) . '" />
			 <input type="hidden" name="post_type" value="' . $post_type . '" />
			 <input type="hidden" name="taxonomy" value="' . $taxonomy . '" />
		 </div>
                 <button class="fa fa-search search-button" type="submit" id="searchsubmit"></button>
                 </div>
		</form></div>';
...
if( !function_exists('ftc_ajax_search') ){
	function ftc_ajax_search(){
		...
		if( $results->have_posts() ){
			...
			if( isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count ){
				$view_all_text = sprintf( esc_html__('View all %d results', 'daniel'), $results->found_posts );
...
if(!function_exists('ftc_breadcrumbs')){
	function ftc_breadcrumbs() {
		...
		if ( !empty( $front_id ) ) {
			...
			$home = esc_html__( 'Home', 'daniel' );
		}
		$ar_title = array(
					'search' 		=> esc_html__('Search results for ', 'daniel')
					,'404' 			=> esc_html__('Error 404', 'daniel')
					,'tagged' 		=> esc_html__('Tagged ', 'daniel')
					,'author' 		=> esc_html__('Articles posted by ', 'daniel')
					,'page' 		=> esc_html__('Page', 'daniel')
					,'portfolio' 	=> esc_html__('Portfolio', 'daniel')
					);
...
function ftc_breadcrumbs_title( $show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '' ){
	...
	if( $show_breadcrumb || $show_page_title ){
		...
		echo '<div class="breadcrumb-title-wrapper"><div class="container">';
			if( $show_breadcrumb ){
				ftc_breadcrumbs();
			}
                        echo '<div class="breadcrumb-title" '.$style.'>';
                        if( $show_page_title ){
...
function ftc_register_required_plugins(){
    ...
    $plugins = array(
		...
		,array(
            ...
            'version'            => '5.4.3', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url'       => '', // If set, overrides default API URL and points to an external URL.
        )
