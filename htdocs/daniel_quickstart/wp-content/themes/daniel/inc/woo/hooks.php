...
add_action('woocommerce_after_shop_loop_item', 'ftc_template_loop_short_description_listview', 22);
    ...
    if ($product->managing_stock()) {
        if ($product->is_in_stock() && $product->get_stock_quantity() > get_option('woocommerce_notify_no_stock_amount')) {
            switch (get_option('woocommerce_stock_format')) {
                case 'no_amount' :
                    $availability = esc_html__('In stock', 'daniel');
                    break;
                case 'low_amount' :
                    if ($product->get_stock_quantity() <= get_option('woocommerce_notify_low_stock_amount')) {
                        $availability = sprintf(esc_html__('Only %s left in stock', 'daniel'), $product->get_stock_quantity());
                        if ($product->backorders_allowed() && $product->backorders_require_notification()) {
                            $availability .= ' ' . esc_html__('(can be backordered)', 'daniel');
                        }
                    } else {
                        $availability = esc_html__('In stock', 'daniel');
                    }
                    break;
                default :
                    $availability = sprintf(esc_html__('%s in stock', 'daniel'), $product->get_stock_quantity());
                    if ($product->backorders_allowed() && $product->backorders_require_notification()) {
                        $availability .= ' ' . esc_html__('(can be backordered)', 'daniel');
                    }
                    break;
            ...
            $availability = esc_html__('Available on backorder', 'daniel');
            $class = 'available-on-backorder';
        } elseif ($product->backorders_allowed()) {
            $availability = esc_html__('In stock', 'daniel');
            $class = 'in-stock';
        } else {
            $availability = esc_html__('Out of stock', 'daniel');
            $class = 'out-of-stock';
        }
...
function ftc_template_loop_categories() {
    global $product;
    $categories_label = esc_html__('Categories: ', 'daniel');
    echo wc_get_product_category_list($product->get_id(), ', ', '<span class="product-cate"><span class="list-cat">' . esc_html($categories_label) . '</span>', '</span>');
}
...
function ftc_shop_top_content_widget_area_button() {
    ...
    if (is_active_sidebar('product-category-top-content') && $smof_data['ftc_prod_cat_top_content']) {
        ?>
        <div class="prod-cat-show-top-content-button"><a href="#"><?php esc_html_e('Filter', 'daniel') ?></a></div>
        <?php
    }
}
...
add_action('woocommerce_single_product_summary', 'ftc_template_single_availability', 7);
...
function ftc_template_single_navigation() {
    ...
    <?php esc_html_e('Send to a Friend', 'daniel') ?>
            </a>
        </div>
        <div class="print">
            <a href="javascript:window.print()" rel="nofollow"><i class="fa fa-print"></i><?php esc_html_e('Print', 'daniel') ?></a>
        </div>
    <?php
}
...
function ftc_template_after_single_social_sharing() {
    ...
    function ftc_template_single_meta() {
        ...
        if ($smof_data['ftc_prod_cat']) {
            echo wc_get_product_category_list($product->get_id(), ', ', '<div class="caftc-link"><span>' . _n('Category:', 'daniel', count($product->get_category_ids()), 'daniel') . ' ', '</span><span class="cat-links"></span></div>');
        }
        if ($smof_data['ftc_prod_tag']) {
            echo wc_get_product_tag_list($product->get_id(), ', ', '<div class="tags-link"><span>' . _n('Tag:', 'Tags:', count($product->get_tag_ids()), 'daniel') . ' ', '</span><span class="tag-links"></span></div>');
        }
    ...
    function ftc_template_single_sku() {
        ...
        if (wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type('variable') )) {
            echo '<div class="sku-wrapper product_meta">' . esc_html__('Sku: ', 'daniel') . '<span class="sku" itemprop="sku">' . (( $sku = $product->get_sku() ) ? $sku : esc_html__('N/A', 'daniel')) . '</span></div>';
        }
    }
    ...
    function ftc_template_single_availability() {
        ...
        $product_stock = ftc_product_get_availability();
        $availability_text = empty($product_stock['availability']) ? esc_html__('In Stock', 'daniel') : esc_attr($product_stock['availability']);
        ?>	
 ...
 * End tag:   	10005
 * ************************************************************ */
add_action('woocommerce_after_shop_loop_item_title', 'ftc_template_loop_add_to_cart', 10004);
...
function ftc_product_group_button_start() {
    ...
    if ($smof_data['ftc_effect_hover_product_style'] != 'style-3') {
        ...
        remove_action('woocommerce_after_shop_loop_item_title', 'ftc_template_loop_add_to_cart', 10004);
    }
...
if (class_exists('YITH_WCWL')) {
    ...
    function ftc_add_wishlist_button_to_product_list() {
        ...
        . '" class="add_to_wishlist yith-wcwl-add-to-wishlist" ><i class="fa fa-heart"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'daniel') . '</span></a>';
        ...
        echo '<a href="' . esc_url($wishlist_url) . '"><i class="fa fa-heart-o"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'daniel') . '</span></a>';
        echo '</span>';
...
if (class_exists('YITH_Woocompare') && get_option('yith_woocompare_compare_button_in_products_list') == 'yes') {
    ...
    if ($yith_woocompare->is_frontend() || $is_ajax) {
        ...
        add_action('woocommerce_after_shop_loop_item_title', 'ftc_add_compare_button_to_product_list', 10002);
...
function ftc_cart_continue_shopping_button() {
    echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="button button-secondary">' . esc_html__('Continue Shopping', 'daniel') . '</a>';
}
?>