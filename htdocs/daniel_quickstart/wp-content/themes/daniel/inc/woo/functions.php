...
if( !function_exists('ftc_tiny_account') ){
	function ftc_tiny_account(){
		...
		<div class="ftc-account">
			<div class="ftc_login">
                                <span class="icon-ftc_login"></span>
				<?php if( !$_user_logged ): ?>
					<a  class="login" href="<?php echo esc_url($login_url); ?>" title="<?php esc_html_e('Login','daniel'); ?>"><span><?php esc_html_e('Login','daniel');?></span></a>
					 / 
					<a class="ftc_sign_up" href="<?php echo esc_url($register_url); ?>" title="<?php esc_html_e('Create New Account','daniel'); ?>"><span><?php esc_html_e('Sign up','daniel');?></span></a>
				<?php else: ?>
					<ul class="daniel-login">
					<li><a class="my-account" href="<?php echo esc_url($profile_url); ?>" title="<?php esc_html_e('My Account','daniel'); ?>"><span><?php esc_html_e('My Account','daniel');?></span></a></li>

					<li><a class="log-out" href="<?php echo esc_url($logout_url); ?>" title="<?php esc_html_e('Logout','daniel'); ?>"><span><?php esc_html_e('Logout','daniel');?></span></a></li>
					</ul>
				<?php endif; ?>
			</div>
			<?php if( !$_user_logged ): ?>
			<div class="ftc_account_form dropdown-container">
					<form name="ftc-login-form" class="ftc-login-form" action="<?php echo esc_url(wp_login_url()); ?>" method="post">
						...
						<p class="login-username">
							<label><?php esc_html_e('Username', 'daniel'); ?></label>
							<input type="text" name="log" class="input" value="" size="20" autocomplete="off">
						</p>
						<p class="login-password">
							<label><?php esc_html_e('Password', 'daniel'); ?></label>
							<input type="password" name="pwd" class="input" value="" size="20">
						</p>
						
						<p class="login-submit">
							<input type="submit" name="wp-submit" class="button-secondary button" value="<?php esc_html_e('Login', 'daniel'); ?>">
							<input type="hidden" name="redirect_to" value="<?php echo esc_url($redirect_to); ?>">
						</p>
						
					</form>
		
					<p class="ftc_forgot_pass"><a href="<?php echo esc_url(wp_lostpassword_url()); ?>" title="<?php esc_html_e('Forgot Your Password?','daniel');?>"><?php esc_html_e('Forgot Your Password?','daniel');?></a></p>
			</div>
			<?php endif; ?>
		</div>
	...
	function ftc_tiny_cart(){
		...
		?>
			<div class="ftc-shoppping-cart">
				<a class="ftc_cart" href="<?php echo esc_url($cart_url); ?>" title="<?php esc_html_e('View your shopping bag','daniel');?>">
				...
				<div class="ftc_shopping_form dropdown-container">
						<?php if( $cart_empty ): ?>
							<label><?php esc_html_e('Your shopping cart is empty', 'daniel'); ?></label>
						<?php else: ?>
								...
								?>
									<li>
										...
										<div class="cart-item-wrapper">	
											...
											<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s" data-key="%s">&times;</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), esc_html__( 'Remove this item', 'daniel' ), $cart_item_key ), $cart_item_key ); ?>
										</div>
									</li>
							...
							<div class="ftc_cart_check">
								<div class="total"><span class="total-title"><?php esc_html_e('Subtotal :', 'daniel');?></span><?php echo WC()->cart->get_cart_subtotal(); ?> </div>
								
								<a href="<?php echo esc_url($cart_url); ?>" class="button view-cart"><?php esc_html_e('View cart', 'daniel'); ?></a>
								<a href="<?php echo esc_url($checkout_url); ?>" class="button checkout button-secondary"><?php esc_html_e('Checkout', 'daniel'); ?></a>
							</div>
						<?php endif; ?>
				</div>
			</div>
...
function ftc_tini_wishlist(){
	...
	<a title="<?php  esc_html_e('Wishlist','daniel'); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="tini-wishlist">
		<?php esc_html_e('Wishlist','daniel'); ?> <?php echo '('.($count > 0?zeroise($count, 2):'0').')'; ?>
	</a>
			...
			foreach( $currencies as $currency ){
				if($woocommerce_wpml->settings['currency_options'][$currency]['languages'][$sitepress->get_current_language()] == 1 ){
					...
					if( $currency == $woocommerce_wpml->multi_currency->get_client_currency() ){
						$selected_html = '<div class="ftc-currency"><a href="javascript: void(0)" class="ftc-currency-selector">'.$currency_format.'</a>';
						break;
					}
				}
			}
