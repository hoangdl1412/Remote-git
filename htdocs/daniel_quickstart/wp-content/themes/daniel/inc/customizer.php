...
function ftc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'daniel' ),
		'choices'  => array(
			'light'  => __( 'Light', 'daniel' ),
			'dark'   => __( 'Dark', 'daniel' ),
			'custom' => __( 'Custom', 'daniel' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_control( 'page_layout', array(
		'label'       => __( 'Page Layout', 'daniel' ),
		'section'     => 'theme_options',
		'type'        => 'radio',
		'description' => __( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'daniel' ),
		'choices'     => array(
			'one-column' => __( 'One Column', 'daniel' ),
			'two-column' => __( 'Two Column', 'daniel' ),
		),
		'active_callback' => 'ftc_is_view_with_layout_option',
	) );

	/**
	 * Filter number of front page sections in Daniel.
	 *
	 * @since Daniel 1.0
	 *
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( __( 'Front Page Section %d Content', 'daniel' ), $i ),
			'description'    => ( 1 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'daniel' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
