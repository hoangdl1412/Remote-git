...
function ftc_testimonial_shortcode($atts){
	...
	if( $testimonials->have_posts() ){
		...
		<div class="ftc-sb-testimonial <?php echo ($is_slider)?'ftc-slider loading':''; ?> owl-loaded  owl-carousel" 
			data-nav="<?php echo esc_attr($show_nav) ?>" data-autoplay="<?php echo esc_attr($auto_play) ?>" data-dots="<?php echo esc_attr($show_dots) ?>" data-columns="<?php echo esc_attr($columns) ?>" >
			<?php
...
if( !function_exists('ftc_brands_slider_shortcode') ){
	function ftc_brands_slider_shortcode( $atts, $content = null ){
		...
		?>
		<div class="ftc-sb-brandslider ftc-slider <?php echo esc_attr( implode(' ', $classes) ); ?>" <?php echo implode(' ', $data_attr); ?>>
			<?php if( strlen($title) > 0 ): ?>
...
if( !function_exists('ftc_blogs_shortcode') ){
	function ftc_blogs_shortcode( $atts, $content = null){
		...
		if( $posts->have_posts() ):
			...
			$classes = array();
			$classes[] = 'ftc-sb-blogs ftc-slider';
			
			$atts = compact('title', 'columns', 'per_page', 'categories', 'orderby', 'is_masonry', 'order'
				,'style', 'show_title', 'show_thumbnail', 'show_author'
				,'show_date', 'show_comment', 'show_excerpt', 'show_readmore', 'excerpt_words'
				,'is_slider', 'show_nav', 'auto_play', 'margin', 'show_load_more');
				?>
				<div class="<?php echo esc_attr(implode(' ', $classes)); ?>" data-atts="<?php echo htmlentities(json_encode($atts)); ?>">
					...
					<?php endif; ?>
					<div class="meta-slider">
						<div class="blogs <?php if (!$is_masonry){ ?>owl-loaded owl-carousel <?php } ?>">
							<?php ftc_get_blog_items_content_shortcode($atts, $posts); ?>
						</div>
						<?php if( $show_load_more ): ?>
							<div class="loading-more">
								<a href="#" class="loading-more button" data-paged="2"><?php echo esc_html($load_more_text) ?></a>
							</div>
						<?php endif; ?>
					</div>
				</div>
				...
				<div class="item <?php echo esc_attr($post_format); ?> <?php echo esc_attr($item_class.' '.$item_extra_class) ?>">
					<div class="post-thumbnail">
						...
						if( $post_format == 'gallery' || $post_format === false || $post_format == 'standard' ){
							?>
							<a class="blog-image <?php echo esc_attr($post_format); ?> <?php echo ($post_format == 'gallery')?'loading':''; ?>" href="<?php echo ($post_format == 'gallery')?'javascript: void(0)':get_permalink() ?>">
								<?php 
				...
				<?php if( $post_format != 'quote' ): ?>
					...
					<div class="content-meta">
						<div class="wrp-time-info">
							...
							<?php if( $show_date && $show_thumbnail && ( $post_format == 'gallery' || $post_format === false || $post_format == 'standard' ) ): ?>
								<div class="date-time">
									<i class="fa fa-calendar"></i><span><?php echo get_the_time('M'); ?> <?php echo get_the_time('d'); ?> , <?php echo get_the_time('Y'); ?></span>
								</div>
							<?php endif; ?>

							<!-- Post Author -->
							<?php if( $show_author ): ?>
								<span class="byline"><i class="fa fa-user" aria-hidden="true"></i><?php esc_html_e('','themeftc') ?><?php the_author_posts_link(); ?></span>
							<?php endif; ?>
						...
						<header class="entry-header">
							<!-- Entry header -->
							<?php if( $show_title ): ?>
								<h4 class="entry-title title_sub"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php the_title(); ?></a></h4>
							<?php endif; ?>
						</header>
	...
	if( !function_exists('ftc_products_shortcode') ){
		...
		function ftc_products_shortcode($atts, $content){
			...
			$classes = array();
			$classes[] = 'ftc-products';
			$classes[] = $product_type;
			
			$rand_id = 'ftc-products-'.rand(0, 10000);
	...
	if( !function_exists('ftc_products_slider_shortcode') ){
		function ftc_products_slider_shortcode($atts, $content){
			...
			$classes = array();
			$classes[] = 'ftc-product-slider';
			$classes[] = $product_type;
			$classes[] = 'ftc-slider';

			if( $products->have_posts() ): 
				?>
			<div class="<?php echo esc_attr(implode(' ', $classes)); ?>" data-is_slider="<?php echo esc_attr((int)$is_slider) ?>" data-columns="<?php echo esc_attr($columns) ?>"
				data-nav="<?php echo esc_attr($show_nav) ?>" data-margin="<?php echo esc_attr($margin) ?>">
				<div class="header-title">
					<?php if( strlen($title) > 0 ): ?>
	...
	if( !function_exists('ftc_product_deals_slider_shortcode') ){
		function ftc_product_deals_slider_shortcode($atts, $content = null){
			...
			?>
			<div class="ftc-product-time-deal <?php echo esc_attr( implode(' ', $classes) ); ?> ftc-slider" <?php echo implode(' ', $data_attr); ?>>
				<div class="header-title">
					<?php if( strlen($title) > 0 ): ?>
						<h3 class="title_sub"><span><?php echo esc_html($title); ?></span></h3>
					<?php endif; ?>
...
//                var_dump($image_size);
	...
	function ftc_soundcloud_iframe_widget($options) {
		...
		return '<div class="ftc-soundcloud '.$unique_class.'" ><iframe style="'.$style.'" src="'.esc_url( $url ).'"></iframe></div>';
	}
