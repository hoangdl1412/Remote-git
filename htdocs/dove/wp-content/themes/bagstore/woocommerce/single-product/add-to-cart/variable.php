...
$attr_dropdown = !isset($smof_data['vela_prod_attr_dropdown']) || ( isset($smof_data['vela_prod_attr_dropdown']) && $smof_data['vela_prod_attr_dropdown'] );
$select_class = '';
if( !$attr_dropdown ){
	$select_class = 'hidden';
}
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php esc_html_e( 'This product is currently out of stock and unavailable.', 'bagstore' ); ?></p>
	<?php else : ?>
				...
				<?php foreach ( $attributes as $attribute_name => $options ) : ?>
						...
						<div class="value">
							...
							<?php if( !$attr_dropdown && is_array( $options ) ): ?>
								<div class="vela-product-attribute">
									<?php 
									...
									if ( taxonomy_exists( $attribute_name ) ) {
										...
										foreach ( $terms as $term ) {
											...
											if( $is_attr_color ){
												$datas = get_term_meta( $term->term_id, 'vela_product_color_config', true );
												if( strlen( $datas ) > 0 ){
													$datas = unserialize( $datas );	
												}else{
													$datas = array(
																'vela_color_color' 				=> "#ffffff"
																,'vela_color_image' 				=> 0
															);
											...
											if( $is_attr_color ){
												if( absint($datas['vela_color_image']) > 0 ){
													echo '<a href="#">' . wp_get_attachment_image( absint($datas['vela_color_image']), 'vela_prod_color_thumb', true, array('title'=>$term->name, 'alt'=>$term->name) ) . '</a>';
												}
												else{
													echo '<a href="#" style="background-color:' . $datas['vela_color_color'] . '">' . apply_filters( 'woocommerce_variation_option_name', $term->name ) . '</a>';
												}
											}
											else{
							...
							echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear selection', 'bagstore' ) . '</a>' ) : '';
						?>
						</div>
                                            </div>
