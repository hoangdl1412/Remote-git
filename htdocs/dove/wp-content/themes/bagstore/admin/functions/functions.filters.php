...
function vela_filter_load_background_image( $bg_url ){
	if( is_string($bg_url) ){
		$bg_url = str_replace(
					array(
						'[site_url]', 
...
add_filter('theme_mod_background_image', 'vela_filter_load_background_image');

/* Allow upload font files */
if( is_admin() ){
	global $pagenow;
	if( $pagenow == 'themes.php' && isset($_GET['page']) && $_GET['page'] == 'optionsframework' ){
		add_filter('upload_mimes', 'vela_allow_upload_font_files');
		function vela_allow_upload_font_files( $existing_mimes = array() ){
