...
function vela_of_option_setup()	
{
	global $of_options, $options_machine;
	$vela_data = of_get_options();
	$options_machine = new Options_Machine($of_options);
	$vela_data = vela_array_atts($options_machine->Defaults, $vela_data);
	vela_of_save_options($vela_data);	
}

function vela_of_save_options($data){
	foreach ( $data as $k=>$v ) {
		if (is_array($v)) {
			foreach ($v as $key=>$val) {
				if ($key != $k && $v[$key] == $val) {
