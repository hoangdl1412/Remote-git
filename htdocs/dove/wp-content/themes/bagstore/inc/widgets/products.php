<?php
add_action('widgets_init', 'vela_products_load_widgets');

function vela_products_load_widgets()
{
	register_widget('Vela_Products_Widget');
}

if( !class_exists('Vela_Products_Widget') ){
	class Vela_Products_Widget extends WP_Widget {

		function __construct() {
			$widgetOps = array('classname' => 'vela-items-widget', 'description' => esc_html__('Display your products on site','bagstore'));
			parent::__construct('vela_products', esc_html__('Vela - Products','bagstore'), $widgetOps);
		}
		...
		function widget( $args, $instance ) {
			...
			$lazy_load = isset($smof_data['vela_prod_lazy_load']) && $smof_data['vela_prod_lazy_load'];
			...
			if( $products->have_posts() ){
				...
				$extra_class = '';
				$extra_class .= ($is_slider)?'vela-slider  owl-carousel owl-loaded ':'';                                                            
				?>
				
                                <div class="vela-product-items-widget woocommerce <?php echo esc_attr($extra_class); ?>" <?php if($is_slider ){ ?> data-show_nav="<?php echo esc_attr($show_nav) ?>" data-auto_play="<?php echo esc_attr($auto_play) ?>"<?php } ?>>
					<?php while( $products->have_posts() ): $products->the_post(); $product = wc_get_product( $post->ID ); ?>
						<?php if( $count % $row == 0 ): ?>
						<ul class="product_list_widget">
						<?php endif; ?>
							<li>
								<a class="vela-widget-image" href="<?php echo esc_url( get_permalink($product->get_id()) ); ?>" title="<?php echo esc_attr( $product->get_title() ); ?>">
									<?php  
										if ( $show_thumbnail ) {
											if( !$lazy_load ){
												...
												echo '<img src="'.esc_url($placeholder_img_src).'" data-src="'.esc_url($img_src).'" alt="'.esc_attr($alt).'" class="attachment-shop_thumbnail wp-post-image vela-lazy-load" width="'.$dimensions['width'].'" height="'.$dimensions['height'].'" />';
											}
										}
									?>
								</a>
								
								<div class="vela-meta-widget product-meta">
                                                                        <?php 
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('product_type'); ?>"><?php esc_html_e('Product type', 'bagstore'); ?> </label>
				<select class="widefat" id="<?php echo $this->get_field_id('product_type'); ?>" name="<?php echo $this->get_field_name('product_type'); ?>">
					<option value="recent" <?php selected($instance['product_type'], 'recent'); ?>><?php esc_html_e('Recent', 'bagstore'); ?></option>
					<option value="sale" <?php selected($instance['product_type'], 'sale'); ?>><?php esc_html_e('Sale', 'bagstore'); ?></option>
					<option value="featured" <?php selected($instance['product_type'], 'featured'); ?>><?php esc_html_e('Featured', 'bagstore'); ?></option>
					<option value="best_selling" <?php selected($instance['product_type'], 'best_selling'); ?>><?php esc_html_e('Best selling', 'bagstore'); ?></option>
					<option value="top_rated" <?php selected($instance['product_type'], 'top_rated'); ?>><?php esc_html_e('Top rated', 'bagstore'); ?></option>
					<option value="mixed_order" <?php selected($instance['product_type'], 'mixed_order'); ?>><?php esc_html_e('Mixed order', 'bagstore'); ?></option>
				</select>
			</p>
			
			<p>
				<label><?php esc_html_e('Select categories', 'bagstore'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('row'); ?>"><?php esc_html_e('Number of rows - in carousel slider', 'bagstore'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('limit'); ?>"><?php esc_html_e('Number of posts to show', 'bagstore'); ?> </label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_thumbnail'); ?>"><?php esc_html_e('Show thumbnail', 'bagstore'); ?></label>
			</p>
                        
			<p>
				<label for="<?php echo $this->get_field_id('thumbnail_size'); ?>"><?php esc_html_e('Thumbnail size', 'bagstore'); ?> </label>
				<select class="widefat" id="<?php echo $this->get_field_id('thumbnail_size'); ?>" name="<?php echo $this->get_field_name('thumbnail_size'); ?>">
					<option value="shop_thumbnail" <?php selected($instance['thumbnail_size'], 'shop_thumbnail'); ?>><?php esc_html_e('Product Thumbnails', 'bagstore'); ?></option>
					<option value="shop_catalog" <?php selected($instance['thumbnail_size'], 'shop_catalog'); ?>><?php esc_html_e('Catalog Images', 'bagstore'); ?></option>
					<option value="shop_single" <?php selected($instance['thumbnail_size'], 'shop_single'); ?>><?php esc_html_e('Single Product Image', 'bagstore'); ?></option>
				</select>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_categories'); ?>"><?php esc_html_e('Show categories', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_product_title'); ?>"><?php esc_html_e('Show product title', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_price'); ?>"><?php esc_html_e('Show price', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_rating'); ?>"><?php esc_html_e('Show rating', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('is_slider'); ?>"><?php esc_html_e('Show in a carousel slider', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_nav'); ?>"><?php esc_html_e('Show navigation button', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('auto_play'); ?>"><?php esc_html_e('Auto play', 'bagstore'); ?></label>
			</p>
