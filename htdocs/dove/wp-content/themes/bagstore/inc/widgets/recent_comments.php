<?php
add_action('widgets_init', 'vela_recent_comments_load_widgets');

function vela_recent_comments_load_widgets()
{
	register_widget('Vela_Recent_Comments_Widget');
}

if( !class_exists('Vela_Recent_Comments_Widget') ){
	class Vela_Recent_Comments_Widget extends WP_Widget {

		function __construct() {
			$widgetOps = array('classname' => 'vela-recent-comments-widget', 'description' => esc_html__('Display recent comments on site', 'bagstore'));
			parent::__construct('vela_recent_comments', esc_html__('Vela - Recent Comments', 'bagstore'), $widgetOps);
		}
		...
		function widget( $args, $instance ) {
			...
			if( $comments ):
				...
				?>
				<div class="vela-recent-comments-widget-wrapper <?php echo esc_attr($extra_class); ?>" data-show_nav="<?php echo esc_attr($show_nav) ?>" data-auto_play="<?php echo esc_attr($auto_play) ?>">
					<?php foreach( (array) $comments as $comment ): $GLOBALS['comment'] = $comment; ?>
						...
						<?php endif; ?>
							<li>
								...
								<?php if( $show_comment ): ?>
								<blockquote class="comment-body"><?php echo vela_string_limit_words(wp_strip_all_tags(get_comment_text()), 15); ?></blockquote>
								<?php endif; ?>
							</li>
		...
		function form( $instance ) {
			...
			$post_types = array_merge(array('all'=>esc_html__('All Posts', 'bagstore')), $post_types);
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('post_type'); ?>"><?php echo esc_html_e('Post type', 'bagstore'); ?> </label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('limit'); ?>"><?php esc_html_e('Number of comments to show', 'bagstore'); ?> </label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_avatar'); ?>"><?php esc_html_e('Show avatar', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_date'); ?>"><?php esc_html_e('Show comment date', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_author'); ?>"><?php esc_html_e('Show comment author', 'bagstore'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_comment'); ?>"><?php esc_html_e('Show comment content', 'bagstore'); ?></label>
			</p>
