<?php 
global $smof_data;
if( !empty($smof_data['vela_enable_quickshop']) && vela_has_woocommerce() && !class_exists('Vela_Quickshop') && !wp_is_mobile() ){
		
	class Vela_Quickshop{
		...
		function add_quickshop_button(){
			...
			$href = admin_url('admin-ajax.php', is_ssl()?'https':'http') . '?ajax=true&action=load_quickshop_content&product_id='.$product->get_id();
				echo '<a class="quickshop" href="'.esc_url($href).'"><i class="fa fa-eye"></i><span class="vela-note">'.esc_html__('Quick view', 'bagstore').'</span></a>';
		}
		...
		function add_hook(){
			...
			add_action('vela_quickshop_single_product_title', array($this, 'product_title'), 10);
			add_action('vela_quickshop_single_product_summary', 'woocommerce_template_single_rating', 10);
			add_action('vela_quickshop_single_product_summary', 'vela_template_single_sku', 20);
			add_action('vela_quickshop_single_product_summary', 'vela_template_single_availability', 30);
			add_action('vela_quickshop_single_product_summary', 'woocommerce_template_single_excerpt', 40);
			add_action('vela_quickshop_single_product_summary', 'woocommerce_template_single_price', 50);
			if( !$smof_data['vela_enable_catalog_mode'] ){
				add_action('vela_quickshop_single_product_summary', 'woocommerce_template_single_add_to_cart', 60); 
			}
		...
		function load_quickshop_content_callback(){
			...
			add_filter( 'vela_woocommerce_review_link_filter', array($this, 'filter_review_link'), 10 );
			
			$_wrapper_class = "vela-quickshop-wrapper product type-{$product->get_type()}";
			ob_start();	
			?>		
			<div itemscope itemtype="http://schema.org/Product" id="product-<?php echo get_the_ID();?>" <?php post_class( apply_filters('single_product_wrapper_class',$_wrapper_class  ) ); ?>>
				...
				<div class="summary entry-summary">
					<?php do_action('vela_quickshop_single_product_title'); ?>
					<?php do_action('vela_quickshop_single_product_summary'); ?>
				</div>
			...
			remove_filter( 'vela_woocommerce_review_link_filter', array($this, 'filter_review_link'), 10 );
