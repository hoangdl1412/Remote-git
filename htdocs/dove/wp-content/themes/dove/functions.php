<?php
/**
 * dove functions and definitions
 *
...
/**
 * dove only works in WordPress 4.7 or later.
 */
...
function ftc_custom_sidebar_form(){
	...
	<form action="<?php echo admin_url( 'widgets.php' ); ?>" method="post" id="ftc-form-add-sidebar">
		<input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'dove') ?>" />
		<button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'dove') ?></button>
	</form>
	<?php
}
...
function ftc_setup() {
	 ...
	 * If you're building a theme based on dove, use a find and replace
	 * to change 'dove' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'dove' );
	...
	/* Translation */
	load_theme_textdomain( 'dove', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary'    => __( 'Primary Navigation', 'dove' ),
		'vertical' => __( 'Vertical Navigation', 'dove' ),
		) );
	...
	$starter_content = array(
		...
		'attachments' => array(
			'image-espresso' => array(
				'post_title' => _x( 'Espresso', 'Theme starter content', 'dove' ),
				'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
				),
			'image-sandwich' => array(
				'post_title' => _x( 'Sandwich', 'Theme starter content', 'dove' ),
				'file' => 'assets/images/sandwich.jpg',
				),
			'image-coffee' => array(
				'post_title' => _x( 'Coffee', 'Theme starter content', 'dove' ),
		...
		'nav_menus' => array(
			...
			'social' => array(
				'name' => __( 'Social Links Menu', 'dove' ),
				'items' => array(
					'link_yelp',
					'link_facebook',
					'link_twitter',
	...
	/**
	 * Filters dove array of starter content.
	 *
	 * @since dove 1.1
	 *
...
function ftc_content_width() {
	...
	/**
	 * Filter dove content width of the theme.
	 *
	 * @since dove 1.0
	 *
...
function ftc_fonts_url() {
	 ...
	 */
	$dosis = _x( 'on', 'Dosis font: on or off', 'dove' );
...
function ftc_excerpt_more( $link ) {
	...
	$link = sprintf( '<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>',
		...
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'dove' ), get_the_title( get_the_ID() ) )
		);
	return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
	...
	if ( has_nav_menu( 'top' ) ) {
		...
		$ftc_l10n['expand']         = __( 'Expand child menu', 'dove' );
		$ftc_l10n['collapse']       = __( 'Collapse child menu', 'dove' );
		$ftc_l10n['icon']           = ftc_get_svg( array( 'icon' => 'angle-down', 'fallback' => true ) );
	}

	wp_enqueue_script( 'ftc-global', get_theme_file_uri( '/assets/js/global.js' ), array( 'jquery' ), '1.0', true );
		...
		<div class="logo">
			<a href="<?php echo esc_url( home_url('/') ); ?>">
				...
				<?php if( strlen($logo_image) > 0 ): ?>
					<img src="<?php echo esc_url($logo_image); ?>" alt="<?php echo !empty($logo_text) ? esc_attr($logo_text) : get_bloginfo('name'); ?>" title="<?php echo !empty($logo_text) ? esc_attr($logo_text) : get_bloginfo('name'); ?>" class="ftc_logo" />
				<?php endif; ?>
...
if( !function_exists('ftc_get_search_form_by_category') ){
	function ftc_get_search_form_by_category(){
		...
		if( $search_for_product ){
			...
			$post_type = 'product';
			$placeholder_text = esc_html__('Keyword ...', 'dove');
		}
		else{
			$taxonomy = 'category';
			$post_type = 'post';
			$placeholder_text = esc_html__('Search', 'dove');
		}
		
		$options = '<option value="">'.esc_html__('Our categories', 'dove').'</option>';
		...
		<form method="get" action="' . esc_url( home_url( '/'  ) ) . '">
			...
			<div class="ftc_search_ajax">
				...
				<input type="submit" title="' . esc_attr__( 'Search', 'dove' ) . '" value="' . esc_attr__( 'Search', 'dove' ) . '" />
	...
	function ftc_ajax_search(){
		...
		if( $results->have_posts() ){
			...
			while( $results->have_posts() ){
				...
				$html .= '<p class="description">'. ftc_the_excerpt_max_words($desc_limit_words, '', true, ' ...', false) .'</p>';
				if( $post_type == 'product' ){
			...
			if( isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count ){
				$view_all_text = sprintf( esc_html__('View all %d results', 'dove'), $results->found_posts );
				
				$html .= '<div class="ftc_search_all">';
...
if(!function_exists('ftc_breadcrumbs')){
	function ftc_breadcrumbs() {
		...
		if ( !empty( $front_id ) ) {
			...
			$home = esc_html__( 'Home', 'dove' );
		}
		$ar_title = array(
			'search' 		=> esc_html__('Search results for ', 'dove')
			,'404' 			=> esc_html__('Error 404', 'dove')
			,'tagged' 		=> esc_html__('Tagged ', 'dove')
			,'author' 		=> esc_html__('Articles posted by ', 'dove')
			,'page' 		=> esc_html__('Page', 'dove')
			,'portfolio' 	=> esc_html__('Portfolio', 'dove')
			);
...
function ftc_breadcrumbs_title( $show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '' ){
	...
	if( $show_breadcrumb || $show_page_title ){
		...
		echo '<div class="ftc-breadcrumb" '.$style.'><div class="container"><div class="breadcrumb-title">';
		if( $show_page_title ){
			echo '<h3 class="title_sub page-title entry-title '.$extra_class_title.'">'.$page_title.'</h3>';
		}
		if( $show_breadcrumb ){
			ftc_breadcrumbs();
		}
...
function ftc_breadcrumbs_titles( $show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '' ){
	global $smof_data;
	if( $show_breadcrumb || $show_page_title ){
		$breadcrumb_bg = '';
		if( $smof_data['ftc_enable_breadcrumb_background_image'] ){
